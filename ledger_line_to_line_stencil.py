...
class _Insert(InsertStmt) :
  def __init__(self, ledger_line, n_lines, staff_space, staff_symbol, note_head_width, y_position, line_stencil) :
  ...
  def _generate_stmt(self, id) :
    ...
    ledger_line_to_line_stencil = select([
      ...
      (note_head_width.c.val + 1.0).label('x1'),
    ...
    where(note_head_width.c.id == staff_symbol.c.id).\
    ...
    ledger_line_to_line_stencil = ledger_line_to_line_stencil.union_all(
     ...
     where(ledger_line_to_line_stencil_prev.c.sub_id < func.abs(ledger_line.c.val))
    )
...
def generate_ddl(name, ledger_line, n_lines, staff_space, staff_symbol, note_head_width, y_position, line_stencil) :
  OUT = []

  insert_stmt = _Insert(ledger_line, n_lines, staff_space, staff_symbol, note_head_width, y_position, line_stencil)

  del_stmt = _Delete(name, line_stencil)

  OUT += [DDL_unit(table, action, [del_stmt], [insert_stmt])
     for action in ['INSERT', 'UPDATE', 'DELETE']
     for table in [ledger_line, note_head_width, staff_symbol, y_position]]
...
if __name__ == "__main__" :
  ...
  for x in range(1,20) :
    ...
    stmts.append((Note_head_width, {'id':x,'val':1.34}))
