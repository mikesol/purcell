...
class _Insert(InsertStmt) :
  def __init__(self, stem_direction, stem_length, staff_symbol, staff_space, stem_end) :
  ...
  def _generate_stmt(self, id) : 
    ...
    stem_to_stem_end = select([
      stem_direction.c.id.label('id'),
      (stem_direction.c.val * stem_length.c.val * staff_space.c.val).label('val'),
    ]).\
      ...
      where(staff_symbol.c.id == stem_direction.c.id).\
    cte(name='stem_to_stem_end')

    self.register_stmt(stem_to_stem_end)

    self.insert = simple_insert(stem_end, stem_to_stem_end)

def generate_ddl(stem_direction, stem_length, staff_symbol, staff_space, stem_end) :
  OUT = []

  insert_stmt = _Insert(stem_direction, stem_length, staff_symbol, staff_space, stem_end)

  del_stmt = _Delete(stem_end)

  OUT += [DDL_unit(table, action, [del_stmt], [insert_stmt])
     for action in ['INSERT', 'UPDATE', 'DELETE']
     for table in [stem_direction, stem_length, staff_symbol]]
